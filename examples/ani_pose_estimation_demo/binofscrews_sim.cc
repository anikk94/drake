...
Drake maintainers should keep this file in sync with ani_pose_estimation_demo.py. */
...
#include "drake/examples/ani_pose_estimation_demo/scenario.h"
...
DEFINE_string(scenario_file, "",
              ...
              "drake/examples/ani_pose_estimation_demo/example_scenarios.yaml");
...
//   std::cout << "rotation matrix:\n" << R.format(fmt) << std::endl;
// }

// void print_clock(
//   const drake::systems::Context<double>& plant_context
// ) {
//   std::cout << "sim time: " << plant_context.get_time() << std::endl;
// }

void queryPoses(
  const multibody::MultibodyPlant<double>& plant,
  const drake::systems::Context<double>& plant_context) {
    std::cout << "plant_context.get_time() " << plant_context.get_time() << std::endl;
    std::cout << "plant.get_name() " << plant.get_name()  << std::endl;
    ...
    for(drake::multibody::BodyIndex bi(0);bi < plant.num_bodies(); ++bi){
      ...
      std::cout << "[\n";
      for(int i=0;i<3;++i){
        std::cout << "  " << rotation_W(i, 0) << ", " << rotation_W(i, 1) << ", " << rotation_W(i, 2) << std::endl;
      }
      std::cout << "]\n";
    }
...
class Simulation {
 public:
  ...
  // // ANI
  // void print_clock(const drake::systems::Context<double>& plant_context);
 ...
 private:
  ...
  //
  multibody::MultibodyPlant<double>* plant_{nullptr};
  //
...
// void Simulation::print_clock(
//     const drake::systems::Context<double>& plant_context
// ){
//   std::cout << "sim time: " << plant_context.get_time() << "s" << std::endl;
//   std::this_thread::sleep_for(std::chrono::seconds(5));
// }

void Simulation::Simulate() {
  
  // std::thread sim_clock(print_clock, );
  ...
  const drake::systems::Context<double>& final_root_context = simulator_->get_context();
  const multibody::MultibodyPlant<double>& plant = *plant_;
  const drake::systems::Context<double> & final_plant_context = diagram_->GetSubsystemContext(plant, final_root_context);
